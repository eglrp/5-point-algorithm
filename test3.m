...
[R(:,:,1), C(:,:,1), R(:,:,2), C(:,:,2), R(:,:,3), C(:,:,3), R(:,:,4), C(:,:,4)] = CameraPose(E);
[R_true,C_true,n1]=TruePose(R,C,featureExtracted_true{1}(1:2,:),featureExtracted_true{2}(1:2,:),eye(3));
t_true = -R_true*C_true;
[R_true_2,C_true_2,n2]=TruePose_RANSAC(R,C,featureExtracted_true{1}(:,6),featureExtracted_true{2}(:,6),eye(3));

if R_true ~= zeros(3)
figure;
set(gcf,'visible','off');
Draw3DWorld([R_true,t_true], FeatureBag_true, W_T_C1);hold on
